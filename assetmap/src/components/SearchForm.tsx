...
class SearchForm extends React.Component<MyProps, MyState> {
  ...
  public render() {
    ...
    for (const category of this.props.categories) {
      dropdown.push(<option key={category}>{category}</option>);
    }

    for (const column of this.props.columns) {
      tableColumns.push(<th className="border-top-0" key={column} scope="col">{column}</th>);
    }

    for (const row of this.props.data) {
      const cells = []
      for (const cell of row) {
        cells.push(<td key={cell}>{cell}</td>);
      }
      tableRows.push(<tr>{cells}</tr>);
    }

    table.push(<table className="mb-0 table text-center"><thead>{tableColumns}</thead><tbody>{tableRows}</tbody></table>);
      ...
      <form id="SearchForm" className="container mt-5 pt-5">
        <div className="mt-5 pt-5 row">
          <div className="border border-bottom-0 border-dark col-12 col-sm-3 form-group m-0 p-0">
            ...
            <select className="bg-light border-0 custom-select pl-5 py-1 my-2" onChange={this.handleFilter} value={this.state.filterParameter}>
              {dropdown}
            </select>
          </div>
          <div className="border border-bottom-0 border-dark border-left-0 col-12 col-sm-9 form-group m-0 p-0">
            <input className="border-0 bg-light form-control py-1 my-2" onChange={this.handleSearch} placeholder="Search" type="text" value={this.state.searchTerm}/>
          </div>
